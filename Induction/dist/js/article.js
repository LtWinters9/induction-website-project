            ...
            if(XHR.readyState==4) {
                if(XHR.status==200) { // Two stage status check for IE8 support
                    ...
                    if(parseInt(responsedata.mainblogid)!=0 && responsedata.mainblogid) {
                        ...
                        // div.setAttribute("class","articleonly");
                        // create new article
                        var newarticle=document.createElement("article");
                        newarticle.setAttribute("id","a"+responsedata.mainblogid );
                        newarticle.setAttribute("class","inset" );
                        console.log(newarticle);
                        // main.appendChild(div);
                        // div.appendChild(newarticle);
                        main.appendChild(newarticle);
                        // populate new article with content
                        newarticlestr="<h3>"+responsedata.title+"</h3><p style='color:#3d3d3d;'>"+responsedata.blogtext+"</p><footer><p style='color:grey;'>Posted on <time datetime='"+responsedata.blogtime+"'>"+responsedata.blogtime+" by <em>"+responsedata.forename+"</em></p></footer>";
                        ...
                        if((responsedata.currentuser.userid==responsedata.blogposter && responsedata.currentuser.userlevel>1)|| responsedata.currentuser.userlevel>2) {
                            ...
                            deletelink.innerHTML="<a href='deletearticle.php?aID="+responsedata.mainblogid+"' id='db"+responsedata.mainblogid+"'>Delete Post</a></div>";
